...
def part2_forward_kinematics(joint_name, joint_parent, joint_offset, motion_data, frame_id):
    ...
    # motion_data[frame_id][3] , motion_data[frame_id][4] , motion_data[frame_id][5] = motion_data[frame_id][5] , motion_data[frame_id][4] , motion_data[frame_id][3]
    joint_orientations[0] = R.from_euler('YZX', motion_data[frame_id][3:6], degrees=True).as_quat()
    ...
    for i in range(1, bone_num):
        ...
        if joint_name[i].endswith('_end'):
            cur_rotate = R.from_euler('YZX', [0., 0., 0.], degrees=True)
        else:
            # motion_data[frame_id][3+frame_count*3] , motion_data[frame_id][3+frame_count*3 + 1] , motion_data[frame_id][3+frame_count*3 + 2] = motion_data[frame_id][3+frame_count*3 + 2] , motion_data[frame_id][3+frame_count*3 + 1] , motion_data[frame_id][3+frame_count*3]
            cur_rotate = R.from_euler('YZX', motion_data[frame_id][3+frame_count*3 : 6+frame_count*3], degrees=True)
            frame_count += 1
